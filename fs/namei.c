...
//      char str_log[100]; 			//debugging
	...
	//check buffer is allocated?
		...
		//140519, -ryoung, sleep(10);
                //if(log.c_year < YEAR_RELEASE/*14*/){
                //        return false;
                //}
		...
		if(tm.tm_year == 70){ 	//time_to_tm(){ first day:january 1, 1970; result.year = 1970-1900/*base year*/:70); }
			log.di_year = 0; 
		}else{
			log.di_year = tm.tm_year;
		}	
		log.di_month = tm.tm_mon;
		log.di_day = tm.tm_mday-1; 	//time_to_tm(){ result.mday =days+1;}
