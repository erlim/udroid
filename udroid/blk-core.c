...
static bool udroid_io_log(struct request *rq)
{
	...
	if(bio && bio->bi_io_vec && bio->bi_io_vec->bv_page && bio->bi_io_vec->bv_page->mapping){
		...
		if(inode && inode->i_ino != 0 && inode->i_dentry.next){
			...
			//140612 modify ryoung, check dname is not null!
			//luncher, shut down problem
			...
			//140603 modify ryoung, get all log ->filter udroid file later
			//140612 modify ryoung, move 
			//log.ext = get_file_ext(fname);
			log.rwbs = rq->cmd_flags & 0x03;
