...
static int io_parser()
{
	...
	while(1){
		...
		fread(pname, log_size -sizeof(struct uio), 1, read_file);

		sprintf(s_log, "%d %d%d%d%d%d%d %d\t %d\t %d\t %d %d\t %d %d\t %d %d\t %s\n", 
				log_size, log.year,log.month,log.day,log.hour,log.min,log.sec, log.nsec,
